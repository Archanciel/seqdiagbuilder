...
from testclasses.classloopmultinestedloopsimplermissingloopstarttagcaller import ClassLoopMultiNestedLoopSimplerMissingLoopStartTagCaller
from testclasses.classloopmultinestedloopsimplerwithloopendunderifcaller import ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIfCaller
...
@enduml''', commands)
    ...
    def testLoopTagMultiNestedLoopsSimplerWithLoopEndUnderIf(self):
        '''
        This test case tests the correct generation of multi nested
        PlantUml loop.
        '''
        entryPoint = ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIfCaller()

        SeqDiagBuilder.activate(parentdir, 'ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIfCaller', 'call',
                                None)  # activate sequence diagram building
        entryPoint.call('str')

        commands = SeqDiagBuilder.createSeqDiaqCommands('User')

        with open("c:\\temp\\testLoopTagMultiNestedLoopsSimplerWithLoopEndUnderIf.txt", "w") as f:
            f.write(commands)
...
<b><font color=red size=14>  ERROR - ':seqdiag_loop_end' tag located on line 24 of file containing class ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIf is placed on an</font></b>
<b><font color=red size=14>  instruction calling method doMidLoopSimple() which IS NOT part of the execution flow recorded by SeqDiagBuilder.</font></b>
...
actor User
participant ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIfCaller
participant ClassLoopMultiNestedLoopSimplerWithLoopEndUnderIf
participant ClassMidLoop
participant ClassLeaf
@enduml''', commands)
...
@enduml''', commands)
    ...
    def testLoopTagMultiNestedLoopsSimplerMissingLoopStartTag(self):
        '''
        This test case tests the generation of an error warning in the Plant
        Uml command file in case a seqdiag loop end tag has no corresponding
        seqdiag loop start tag.
        '''
        entryPoint = ClassLoopMultiNestedLoopSimplerMissingLoopStartTagCaller()

        SeqDiagBuilder.activate(parentdir, 'ClassLoopMultiNestedLoopSimplerMissingLoopStartTagCaller', 'call',
                                None)  # activate sequence diagram building
        entryPoint.call('str')

        commands = SeqDiagBuilder.createSeqDiaqCommands('User')

        with open("c:\\temp\\testLoopTagMultiNestedLoopsSimplerMissingLoopStartTag.txt", "w") as f:
            f.write(commands)
...
<b><font color=red size=14>  ClassLoopMultiNestedLoopSimplerMissingLoopStartTag. As a consequence, the generated PlantUML sequence diagram commands are syntactically incorrect and</font></b>
<b><font color=red size=14>  were thrown away !</font></b>
<b><font color=red size=14>  To solve the problem, ensure every ':seqdiag_loop_end' tag relates to a corresponding ':seqdiag_loop_start' tag in the method mentioned above.</font></b>
endheader

actor User
participant ClassLoopMultiNestedLoopSimplerMissingLoopStartTagCaller
participant ClassLoopMultiNestedLoopSimplerMissingLoopStartTag
participant ClassMidLoop
participant ClassLeaf
@enduml''', commands)
